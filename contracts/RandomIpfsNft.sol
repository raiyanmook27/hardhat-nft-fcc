...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    event NFTRequested(uint256 indexed _requestId, address indexed requestor);
    event NFTMinted(Breed dogbreed, address _minter);
    ...
    function getBreedFromModdedRng(uint256 moddedRng) public pure returns (Breed) {
        ...
        for (uint256 i = 0; i < chanceArray.length; i++) {
            if (moddedRng >= cumSum && moddedRng < cumSum + chanceArray[i]) {
                return Breed(i);
            }
